...
import {
  ...
  ChartCardProps,
  CustomTooltipProps,
} from './interfaces/player-dashboard.interface'
...
const PlayerDashboard: React.FC<PlayerDashboardProps> = ({
      ...
      >
        ...
        {payload.map((item, index) => (
          <HStack key={index} className="space-x-2">
            <Box className="w-3 h-3 rounded-full" style={{ backgroundColor: item.color }} />
  ...
  const DateSelector = ({
    ...
    <Menu>
      ...
      <MenuList className="bg-gray-800 border-gray-700 max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700">
  ...
  return (
    <Box className="w-full max-w-7xl mx-auto p-6 space-y-8">
      <MotionBox initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <Text className="text-3xl font-bold text-white mb-2">Estatísticas do Jogador</Text>
      ...
      <MotionBox
        ...
        className="bg-gray-900 rounded-xl border border-gray-700 p-6"
      >
        <Text className="text-lg font-semibold text-white mb-4">Histórico de Mortes</Text>
                ...
                >
                  ...
                  <Td className="text-white">
                    <ChakraTooltip label={death.text} hasArrow placement="top">
                      <Text className="truncate max-w-xs cursor-help">{death.text}</Text>
                    </ChakraTooltip>
                  </Td>
                </MotionBox>
              ))}
