...
export const useDeathTable = (deathList: Death[], playAudio: () => void, audioEnabled: boolean) => {
  ...
  useEffect(() => {
    if (shouldResetData()) {
      ...
      useGlobalStore.getState().resetNewCounts();
    }
  }, [shouldResetData]);
  ...
  const handleCopyAllDeaths = useCallback(() => {
    const textToCopy = deathList.map(death =>
      `${death.name}: ${death.text}`
    ).join('\n');
