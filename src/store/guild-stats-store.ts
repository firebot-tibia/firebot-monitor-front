...
export const useGuildStatsStore = create<GuildStatsState>((set, get) => ({
  ...
  setVocationFilter: (vocation) => {
    set({ vocationFilter: vocation, allyGainPage: 1, allyLossPage: 1, enemyGainPage: 1, enemyLossPage: 1 });
    get().fetchGuildStats('ally');
    get().fetchGuildStats('enemy');
  },

  setNameFilter: (name) => {
    set({ nameFilter: name.trim(), allyGainPage: 1, allyLossPage: 1, enemyGainPage: 1, enemyLossPage: 1 });
    get().fetchGuildStats('ally');
    get().fetchGuildStats('enemy');
  },
