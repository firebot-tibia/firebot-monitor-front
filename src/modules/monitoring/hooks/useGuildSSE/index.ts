...
import type {
  ...
  GuildData,
  GuildMemberResponse,
} from '@/core/sse/services/types'
import { useSSEStore } from '@/core/sse/sse-store'
        ...
        switch (message.type) {
          case 'guild_data': {
            ...
            if (Array.isArray(message.data)) {
              ...
              const data = message.data as GuildData
              members = data.members || []
            } else {
              console.warn('[Guild SSE Debug] Invalid guild data format:', message.data)
              return
            }

            if (!Array.isArray(members) || members.length === 0) {
              console.warn('[Guild SSE Debug] No valid members in data')
              return
            }

            console.log('[Guild SSE Debug] Processing guild data:', {
